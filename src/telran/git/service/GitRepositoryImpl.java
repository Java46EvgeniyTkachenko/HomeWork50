...
//	private String ignoreExpressions = "(\\" + GIT_FILE + ")";
	private String ignoreExpressions = "^.\\\\w+";
	...
	public String switchTo(String name) {
		...
		if (info().stream().filter(p -> p.status.equals(Status.COMMITTED)).count() < 1) {
			return res += "SwitchTo may be done only after commit!";
		}
		if (head == name) {
			return res += "Switching to the current commit doesn’t make a sense!";
		}

		return res += " Switch was successful! ";
	}
	...
	public void save() {
		...
		try (
				...
				ObjectOutputStream output = new ObjectOutputStream(Files.newOutputStream(Path.of(gitPath)))) {
